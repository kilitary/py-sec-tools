...
class UI_Hooks(object):
    ...
    def saved(self, *args) -> "void":
        ...
        interface. Note that at the time this notification is sent, the
        internal paths are not updated yet, and calling
        get_path(PATH_TYPE_IDB) will return the previous path.
        
        saved(self, path)
            @param path: the database path  (C++: const char *)
        """
    ...
    def get_custom_viewer_hint(self, *args) -> "PyObject *":
        ...
        IMPORTANT_LINES accordingly. Completely overwriting the existing lines
        in HINT is possible but not recommended. If the REG_HINTS_MARKER
        sequence is found in the returned hints string, it will be replaced
        with the contents of the "regular" hints. If the SRCDBG_HINTS_MARKER
        sequence is found in the returned hints string, it will be replaced
        with the contents of the source-level debugger-generated hints. The
        following keywords might appear at the beginning of the returned
        hints: HIGHLIGHT textwhere text will be highlighted CAPTION
        captioncaption for the hint widget
    ...
    def desktop_applied(self, *args) -> "void":
        ...
        desktop_applied(self, name, from_idb, type)
            @param name: the desktop name  (C++: const char *)
            @param from_idb: the desktop was stored in the IDB (false if it comes
                             from the registry)  (C++: bool)
            @param type: the desktop type (1-disassembly, 2-debugger, 3-merge)
                         (C++: int)
        """
...
"""
When floating or in a splitter (i.e., not tabbed), use the widget's
size hint to determine the best geometry (Qt only)
"""
...
"""
try and move the cursor to a line displaying the 'place_t' if
possible. This might disregard the Y position in case of success
"""
...
"""
activate (i.e., switch to) the viewer. Activation is performed before
the new 'lochist_entry_t' instance is actually copied to the viewer's
'lochist_t' (otherwise, if the viewer was invisible its
on_location_changed() handler wouldn't be called.)
"""

WOPN_RESTORE = _ida_kernwin.WOPN_RESTORE
"""
if the widget was the only widget in a floating area the last time it
was closed, it will be restored as floating, with the same
position+size as before
"""

WOPN_PERSIST = _ida_kernwin.WOPN_PERSIST
"""
widget will remain available when starting or stopping debugger
sessions
"""
...
"""
Place widget into a tab next to dest_ctrl, if dest_ctrl is in a tab
bar (otherwise the same as 'WOPN_DP_INSIDE' )
"""
...
"""
dialog for "Pseudocode" additional flags: 0x01 Library function 0x02
Mark as decompiled
"""
...
class linearray_t(object):
    ...
    def set_place(self, *args) -> "int":
        ...
        Position the array. This function must be called before calling any
        other member functions. 'linearray_t' doesn't own 'place_t'
        structures. The caller must take care of 'place_t' objects.
        
        set_place(self, new_at) -> int
            @param new_at: new position of the array (C++: const  place_t  *)
            @return: the delta of lines that the  linearray_t  had to adjust the
                     place by.   For example, if the  place_t  has a lnnum of 5,
                     but it turns out, upon generating lines,   that the number of
                     lines for that particular place is only 2, then 3 will be
                     returned.
        """
    ...
    def get_place(self, *args) -> "place_t *":
        ...
        Get the current place. If called before 'down()' , then returns place
        of line which will be returned by 'down()' . If called after 'up()' ,
        then returns place if line returned by 'up()' .
        """
    ...
    def get_bg_color(self, *args) -> "bgcolor_t":
        ...
        Get current background color. (the same behavior as with 'get_place()'
        : good before 'down()' and after 'up()' )
        """
    ...
    def get_pfx_color(self, *args) -> "bgcolor_t":
        ...
        Get current prefix color. (the same behavior as with 'get_place()' :
        good before 'down()' and after 'up()' )
        """
    ...
    def get_dlnnum(self, *args) -> "int":
        ...
        Get default line number. (the same behavior as with 'get_place()' :
        good before 'down()' and after 'up()' )
        """
...
"""
triggering a 'edit/rename' (i.e., F2 shortcut) on a cell, should call
the edit() callback for the corresponding row.
"""
...
class chooser_base_t(object):
    ...
    def __init__(self, *args, **kwargs):
        raise AttributeError("No constructor defined - class is abstract")
    __repr__ = _swig_repr
    x0 = property(_ida_kernwin.chooser_base_t_x0_get)
    y0 = property(_ida_kernwin.chooser_base_t_y0_get)
    x1 = property(_ida_kernwin.chooser_base_t_x1_get)
    y1 = property(_ida_kernwin.chooser_base_t_y1_get)
    width = property(_ida_kernwin.chooser_base_t_width_get)
    height = property(_ida_kernwin.chooser_base_t_height_get)
    title = property(_ida_kernwin.chooser_base_t_title_get)
    columns = property(_ida_kernwin.chooser_base_t_columns_get)
    widths = property(_ida_kernwin.chooser_base_t_widths_get)
    header = property(_ida_kernwin.chooser_base_t_header_get)
    icon = property(_ida_kernwin.chooser_base_t_icon_get)
    ...
    popup_names = property(_ida_kernwin.chooser_base_t_popup_names_get)
    deflt_col = property(_ida_kernwin.chooser_base_t_deflt_col_get)
    ...
    def is_same(self, *args) -> "bool":
        ...
        is_same(self, other) -> bool
            @param other (C++: const  chooser_base_t  *)
        """
    ...
    def get_ea(self, *args) -> "ea_t":
        ...
        get an address of an element. Used to set breakpoint in any chooser
        which implements this callback.
    ...
    def get_row(self, *args) -> "PyObject *":
        ...
        get_row(self, n) -> PyObject *
            @param n: element number (0.. get_count() -1) (C++: size_t)
        """
...
class action_ctx_base_cur_sel_t(object):
    ...
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr
    _from = property(_ida_kernwin.action_ctx_base_cur_sel_t__from_get, _ida_kernwin.action_ctx_base_cur_sel_t__from_set)
    to = property(_ida_kernwin.action_ctx_base_cur_sel_t_to_get, _ida_kernwin.action_ctx_base_cur_sel_t_to_set)
...
class action_ctx_base_source_t(object):
    ...
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr
    chooser = property(_ida_kernwin.action_ctx_base_source_t_chooser_get, _ida_kernwin.action_ctx_base_source_t_chooser_set)
...
class action_ctx_base_t(object):
    ...
    cur_sel = property(_ida_kernwin.action_ctx_base_t_cur_sel_get, _ida_kernwin.action_ctx_base_t_cur_sel_set)
    ...
    source = property(_ida_kernwin.action_ctx_base_t_source_get, _ida_kernwin.action_ctx_base_t_source_set)
...
def get_chooser_obj(*args) -> "void *":
    ...
    'ui_get_chooser_obj' ).This attemps to find the choser by its title
    and, if found, returns the result of calling its
    'chooser_base_t::get_chooser_obj()' method.This is object is chooser-
    specific.
