...
class compiled_binpat_vec_t(object):
    ...
    def __eq__(self, *args) -> "bool":
        ...
        __eq__(self, r) -> bool
            r: qvector< compiled_binpat_t > const &
        """
...
def create_align(*args) -> "bool":
    ...
    create_align(ea, length, alignment) -> bool
        @param ea: linear address (C++: ea_t)
        @param length: size of the item in bytes. 0 means to infer from
                       ALIGNMENT (C++: asize_t)
        @param alignment: alignment exponent. Example: 3 means align to 8
...
def calc_min_align(*args) -> "int":
    ...
    calc_min_align(length) -> int
        @param length: size of the item in bytes. (C++: asize_t)
...
def calc_max_align(*args) -> "int":
    ...
    calc_max_align(endea) -> int
        @param endea: end address of the alignment item. (C++: ea_t)
...
def calc_def_align(*args) -> "int":
    ...
    calc_def_align(ea, mina, maxa) -> int
        @param ea: linear address (C++: ea_t)
        @param mina: minimal possible alignment exponent. (C++: int)
        @param maxa: minimal possible alignment exponent. (C++: int)
    """
...
class compiled_binpat_t(object):
    ...
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr
    bytes = property(_ida_bytes.compiled_binpat_t_bytes_get, _ida_bytes.compiled_binpat_t_bytes_set)
    mask = property(_ida_bytes.compiled_binpat_t_mask_get, _ida_bytes.compiled_binpat_t_mask_set)
    strlits = property(_ida_bytes.compiled_binpat_t_strlits_get, _ida_bytes.compiled_binpat_t_strlits_set)
    encidx = property(_ida_bytes.compiled_binpat_t_encidx_get, _ida_bytes.compiled_binpat_t_encidx_set)
...
def parse_binpat_str(*args) -> "qstring *":
    ...
    'in' parameter contains space-separated tokens:
    ...
    a few examples (assuming base 16):
    ...
    parse_binpat_str(out, ea, _in, radix, strlits_encoding=0) -> str
        @param out: a vector of compiled binary patterns, for use with
                    bin_search2() (C++: compiled_binpat_vec_t  *)
        @param ea: linear address to convert for (the conversion depends on
                   the address, because the number of bits in a byte depend on
                   the segment type) (C++: ea_t)
        in: char const *
        @param radix: numeric base of numbers (8,10,16) (C++: int)
        @param strlits_encoding: the target encoding into which the string
                                 ...
                                 the special values PBSENC_* (C++: int)
