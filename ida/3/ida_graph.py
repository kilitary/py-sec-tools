...
class abstract_graph_t(ida_gdl.gdl_graph_t):
    ...
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr
    title = property(_ida_graph.abstract_graph_t_title_get, _ida_graph.abstract_graph_t_title_set)
    rect_edges_made = property(_ida_graph.abstract_graph_t_rect_edges_made_get, _ida_graph.abstract_graph_t_rect_edges_made_set)
    current_layout = property(_ida_graph.abstract_graph_t_current_layout_get, _ida_graph.abstract_graph_t_current_layout_set)
    circle_center = property(_ida_graph.abstract_graph_t_circle_center_get, _ida_graph.abstract_graph_t_circle_center_set)
    circle_radius = property(_ida_graph.abstract_graph_t_circle_radius_get, _ida_graph.abstract_graph_t_circle_radius_set)
    callback_ud = property(_ida_graph.abstract_graph_t_callback_ud_get, _ida_graph.abstract_graph_t_callback_ud_set)
...
class mutable_graph_t(abstract_graph_t):
    ...
    def size(self, *args) -> "int":
        ...
        Get the total number of nodes (including group nodes, and including
        hidden nodes.)See also node_qty()
        """
    ...
    def node_qty(self, *args) -> "int":
        ...
        'gdl.hpp' 's 'node_iterator' )See also size()
        """
    ...
    def exists(self, *args) -> "bool":
        ...
        exists(self, node) -> bool
            @param node: the node number (C++: int)
            @return: success
        """
    ...
    def get_node_representative(self, *args) -> "int":
        ...
        Get the node that currently visually represents 'node'. This will find
        the "closest" parent group node that's visible, by attempting to walk
        up the group nodes that contain 'node', and will stop when it finds a
        node that is currently visible.See also get_group_node()
        
        get_node_representative(self, node) -> int
            @param node: the node (C++: int)
    ...
    def is_visible_node(self, *args) -> "bool":
        ...
        Is the node currently visible?An invisible node is a node that's part
        of a group that's currently collapsed.
    ...
    def create_group(self, *args) -> "int":
        ...
        create_group(self, nodes) -> int
            @param nodes: the nodes that will be part of the group (C++: const
                          intvec_t  &)
    ...
    def delete_group(self, *args) -> "bool":
        ...
        Delete a group node.This deletes the group node only; it does not
        delete nodes that are part of the group.
        
        delete_group(self, group) -> bool
            @param group: the group node (C++: int)
            @return: success
        """
    ...
    def change_group_visibility(self, *args) -> "bool":
        ...
        change_group_visibility(self, group, expand) -> bool
            @param group: the group node (C++: int)
            @param expand: whether to expand or collapse (C++: bool)
            @return: success
        """
    ...
    def add_node(self, *args) -> "int":
        ...
        add_node(self, r) -> int
            @param r: the node geometry (can be nullptr) (C++: const  rect_t  *)
            @return: the new node
        """
    ...
    def del_node(self, *args) -> "ssize_t":
        ...
        del_node(self, n) -> ssize_t
            @param n: the node to delete (C++: int)
    ...
    def refresh(self, *args) -> "bool":
        ...
        Refresh the graphA graph needs refreshing when it's "backing data".
        E.g., if the number (or contents) of the objects in the above example,
        change.Let's say the user's plugin ends up finding a 5th piece of
        scattered data. It should then add it to its internal list of known
        objects, and tell IDA that the graph needs to be refreshed, using
        'refresh_viewer()' . This will cause IDA to:discard all its internal
        rendering information,call 'mutable_graph_t::refresh()' on the graph
        so that the user's plugin has a chance to "sync" the number of nodes &
        edges that this graph contains, to the information that the plugin has
        collected so farre-create internal rendering information, andrepaint
        the view
        """
...
def viewer_delete_groups(*args) -> "bool":
    ...
    Wrapper around 'mutable_graph_t::delete_group' . This function
    will:clone the current graphattempt deleting the groups in that new
    graphif successful, animate to that new graph.
