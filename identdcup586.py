...
try:
	...
	while True:
		...
		while True:
			...
			if len(byte) <= 0 or (len(byte) and (byte == b'\r' or byte == b'\n')):
				break
			buffer.append(byte)
		...
		if buffer.count(',') == 1:
			...
			if len(vars) == 2:
				try:
					...
					fprint(f"identd request src_port: {src_port} dst_port: {dst_port}")
				...
				fprint(f'unsuported or malcrafted format (len(vars)={len(vars)}): {buffer}')
		else:
			fprint(f"unsuported or malcrafted format (,count={buffer.count(',')}): {buffer}")
