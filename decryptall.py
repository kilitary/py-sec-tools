...
if __name__ == '__main__':
	...
	uprint(f"{computer_id}: decryption executed with password: {password}, {len(files)} files")
			...
			with open(file, 'rb') as r:
				try:
					...
					if len(header) != calcsize(Config.HEADER):
						...
						ok += 1
						continue
					sign, version, crc32, file_size, path = unpack(Config.HEADER, header)
					if sign != Config.SIGN:
					...
					if crc != crc32: deb(f'CRC ERROR ', end='')
					else: deb(f'CRC OK ', end='')
