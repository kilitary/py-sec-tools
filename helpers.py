...
def server_message(message):
    if not hasattr(server_message, "sock"):
        server_message.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    try:
        server_message.sock.sendto(bytes(message, encoding="utf-8"), (Config.UDP_IP, Config.UDP_PORT))
