...
if __name__ == '__main__':
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            content = b'\r\n' + (b'A' * secrets.choice(range(1, 15))) + str.encode(
                Randomer.str_str_generator(size=44)) + b"\r\n"
            try:
                sent = s.send(content)
            ...
            if time.time() - log_time >= 1:
                print(
                    f'\r{tot_bytes} ({tot_bytes / 1024.0 / 1024.0:.1f}MB) sent={sent} speed={sec_bytes / 1024.0 / 1024.0:.1f}MB/s',
                    end='')
                sec_bytes = 0
                log_time = time.time()
            else:
